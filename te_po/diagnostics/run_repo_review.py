...
try:
    from te_po.diagnostics import metadata as analysis_metadata
except Exception:
    # Fallback: load local metadata module directly so script works when
    # executed as a standalone file (without package context).
    import importlib.util
    METADATA_PATH = Path(__file__).resolve().parent / "metadata.py"
    spec = importlib.util.spec_from_file_location("analysis_metadata", METADATA_PATH)
    analysis_metadata = importlib.util.module_from_spec(spec)
    try:
        spec.loader.exec_module(analysis_metadata)  # type: ignore
    except Exception:
        # metadata import may require heavy deps; provide lightweight fallback
        class _FallbackMeta:
            def build_metadata(self, resource: str):
                return {"author": "kitenga (fallback)", "resource": resource}

            def write_metadata_file(self, path, resource: str):
                meta = self.build_metadata(resource)
                meta_path = Path(path).with_name(f"{Path(path).stem}.meta.json")
                meta_path.write_text(json.dumps(meta, indent=2, ensure_ascii=False), encoding="utf-8")
                return meta

            def append_markdown_footer(self, path, metadata: dict):
                try:
                    content = Path(path).read_text(encoding="utf-8")
                except Exception:
                    content = ""
                footer = "\n\n---\n"
                footer += f"Author: {metadata.get('author')}\n"
                Path(path).write_text(content + footer, encoding="utf-8")

        analysis_metadata = _FallbackMeta()
